...
class DistanceMatrix:
    ...
    def __init__(self, master_host_dict: Dict[str, Dict[str, Any]]):
        ...
        for genome_id, genome_metadata in master_host_dict.items():
            lineage = tuple(reversed(genome_metadata['lineage_names']))  # TODO - SPECIFIC FOR EDWARDS SET, HAS TO BE CHANGED FOR OTHER JSONS!!!
...
def taxonomic_discordance_sp(distances: DistanceMatrix,
    ...
    for virus_id, predictions in sorted_predictions.items():
        true_host = symbol(master_virus_dict[virus_id]['host']['lineage_names'][-1])  # TODO Edwards notation is VERY CONFUSING (should be master_virus_dict[virus_id]['host']['species/taxid']) !!!
        if len(predictions) > top_n:
            predictions = predictions[:top_n]
...
#     total_start = timer()
#
#     host_json = Path('/home/solificatus/edwards2016/host/host.json')
#     with host_json.open() as hj:
#         host_dict = json.load(hj)
#
#     virus_json = Path('/home/solificatus/edwards2016/virus/virus.json')
#     with virus_json.open() as hj:
#         virus_dict = json.load(hj)
#
#     dists = DistanceMatrix(host_dict)
#
#     with open("/home/solificatus/Desktop/discordance_debug/testname2_powmax.json", 'r') as preds:
